...
//求森林的度
int forest_dot(csNode*& T)
{
	int dot1 = 0;  //当前结点度数 
	int dot2 = 0;  //孩子结点中度数最大值 
	int dot3 = 0;  //兄弟结点度数 
	csNode* p;
	if (T != NULL)
	{
		p = T->firstChild;
		while (p != NULL)  //算出当前结点的度数
		{
			dot1++;
			p = p->nextSibling;
		}
		dot2 = forest_dot(T->firstChild);  //算出子结点中最大的度数
		dot3 = forest_dot(T->nextSibling);  //算出兄弟结点的度数
		if (dot1 < dot2)
			dot1 = dot2;
		if (dot1 < dot3)
			dot1 = dot3;  //找到三者最大值 
	}
	return dot1;  //返回树或森林的度数
}
